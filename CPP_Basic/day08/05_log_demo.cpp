...
int main(int argc, char** argv) {
    ...
    // 输出器对象的名字：OstreamAppender
    // 参数1: 目的地名称 console(控制台)
    // 参数2: 指向 std::cout 的指针

    //这里使用Appender* 使用了父类型指针接收--->多态
    ...
    // 同样为文件输出设置布局（给输出器对象绑定格式化器）
    appender2->setLayout(new log4cpp::BasicLayout());

    // 3. 获取根分类器 (Root Category)
    // Category 是单例模式，通过 getRoot() 或 getInstance() 获取
    // static Category & getRoot ()==>Return the root of the Category hierarchy.
    ...
    */
    ...
    // 将控制台目的地绑定到根分类器
    root.addAppender(appender1);//为记录器添加了一个输出器
